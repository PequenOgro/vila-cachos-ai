...
export default function App() {
  ...
  useEffect(() => {
    ...
    const resetTimer = () => {
      ...
      inactivityTimer = setTimeout(() => {
        // Se a tela não for a de boas-vindas, reseta
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200 flex flex-col justify-between">
      ...
      <header className="border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md px-6 py-4 sticky top-0 z-50">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-2.5 cursor-pointer" onClick={handleReset}>
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-zinc-100 font-serif">Vila Cachos</h1>
              <p className="text-xs text-zinc-400 font-medium">Totem Inteligente de Diagnóstico Capilar</p>
            </div>
          </div>
          ...
          {screen !== 'welcome' && (
            <button
              onClick={handleReset}
              className="flex items-center space-x-1.5 text-xs font-semibold text-zinc-400 hover:text-zinc-100 bg-zinc-800 hover:bg-zinc-700 px-4 py-2 rounded-full transition min-h-[44px]"
            >
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto p-6 flex flex-col justify-center">
        ...
        {screen === 'welcome' && (
          <div className="text-center py-12 px-4 max-w-2xl mx-auto">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-zinc-800 text-zinc-300 text-xs font-semibold mb-6 border border-zinc-800">
              <Sparkles className="w-4 h-4 text-amber-400" />
              Sistema Especialista de Recomendação
            </div>
            
            <h2 className="text-4xl md:text-5xl font-extrabold text-zinc-100 tracking-tight font-serif mb-6 leading-tight">
            ...
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center space-x-3 bg-amber-600 hover:bg-amber-700 text-white font-medium text-lg px-8 py-4 rounded-2xl shadow-xl hover:shadow-2xl transition transform hover:-translate-y-0.5 active:translate-y-0 min-h-[44px]"
            >
              <span>Iniciar Diagnóstico</span>
              <ArrowRight className="w-5 h-5 text-zinc-100" />
            </button>

            <div className="mt-14 grid grid-cols-3 gap-4 text-zinc-400 text-xs font-medium border-t border-zinc-800 pt-8">
              <div>
                <span className="block text-lg font-bold text-zinc-100">100%</span>
                Personalizado para você
              </div>
              <div>
                <span className="block text-lg font-bold text-zinc-100">Híbrido</span>
                Regras + Satisfação Real
              </div>
              <div>
                <span className="block text-lg font-bold text-zinc-100">XAI</span>
        ...
        {/* ------------------------------------------------------------- */}
        {screen === 'anamnese' && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 md:p-10 shadow-sm">
            ...
            {step === 1 && (
              <div>
                <h3 className="text-2xl font-bold text-zinc-100 mb-2">Qual é o tipo de curvatura do seu cabelo?</h3>
                <p className="text-zinc-400 text-sm mb-6">Selecione o padrão predominante nos seus fios.</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    ...
                    <button
                      ...
                      className={`text-left p-5 rounded-2xl border-2 transition relative min-h-[44px] ${
                        formData.curvatura === item.val
                          ? 'border-amber-600 bg-zinc-800/50 shadow-sm'
                    ...
                    >
                      ...
                      <div className="text-2xl font-extrabold text-zinc-100 mb-1">{item.val}</div>
                      <div className="font-bold text-zinc-100 mb-1">{item.title}</div>
            ...
            {step === 2 && (
              <div>
                <h3 className="text-2xl font-bold text-zinc-100 mb-2">Como é a porosidade dos seus fios?</h3>
                <p className="text-zinc-400 text-sm mb-6">A porosidade define a capacidade do cabelo de absorver e reter hidratação.</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    ...
                    >
                      ...
                      <div className="font-bold text-zinc-100 mb-1 text-base">{item.title}</div>
                      <div className="text-xs text-zinc-400 leading-relaxed">{item.desc}</div>
                    </button>
                  ))}
                </div>
            ...
            {step === 3 && (
              <div>
                <h3 className="text-2xl font-bold text-zinc-100 mb-2">Você possui procedimentos químicos nos cabelos?</h3>
                <p className="text-zinc-400 text-sm mb-6">Descoloração, luzes, coloração constante ou relaxamentos químicos antigos.</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    >
                      ...
                      <div className="font-bold text-zinc-100 mb-1 text-lg">{item.title}</div>
                      <div className="text-xs text-zinc-400 leading-relaxed">{item.desc}</div>
                    </button>
                  ))}
                </div>
            ...
            {step === 4 && (
              <div>
                <h3 className="text-2xl font-bold text-zinc-100 mb-2">Quais queixas mais te incomodam hoje?</h3>
                <p className="text-zinc-400 text-sm mb-6">Você pode selecionar mais de uma opção para refinarmos a fórmula exata.</p>
                    ...
                    return (
                      ...
                      >
                        <div className={`w-5 h-5 rounded-md mt-0.5 flex items-center justify-center border transition ${
                          isChecked ? 'bg-amber-600 border-amber-600 text-white' : 'border-zinc-700 bg-zinc-900'
                        }`}>
                          {isChecked && <Check className="w-3.5 h-3.5" />}
                        </div>
                        <div className="flex-1">
                          <div className="font-bold text-zinc-100 text-sm">{q.label}</div>
                          <div className="text-xs text-zinc-400 mt-0.5">{q.desc}</div>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="mt-8 pt-6 border-t border-zinc-800 flex items-center justify-between">
              {step > 1 ? (
                <button
                  ...
                  className="inline-flex items-center justify-center space-x-2 text-zinc-400 hover:text-zinc-100 font-semibold text-sm px-4 py-3 rounded-xl transition min-h-[44px]"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center text-zinc-500 hover:text-zinc-300 font-semibold text-sm px-4 py-3 rounded-xl transition min-h-[44px]"
                >
                  Cancelar
                </button>
              )}
              ...
              {step < 4 ? (
                <button
                  type="button"
                  onClick={() => setStep(prev => prev + 1)}
                  className="inline-flex items-center justify-center space-x-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold text-sm px-6 py-3 rounded-xl transition shadow-md hover:shadow-lg min-h-[44px]"
                >
                  <span>Próximo</span>
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center space-x-2 bg-amber-600 hover:bg-amber-700 text-zinc-100 font-bold text-sm px-6 md:px-8 py-3.5 rounded-xl transition shadow-xl hover:shadow-2xl transform hover:-translate-y-0.5 min-h-[44px]"
                >
        ...
        {screen === 'loading' && (
          <div className="text-center py-20">
            <div className="inline-block w-16 h-16 border-4 border-zinc-800 border-t-amber-600 rounded-full animate-spin mb-6" />
            <h3 className="text-2xl font-bold text-zinc-100 mb-2">Processando Diagnóstico Capilar...</h3>
            ...
            <p className="text-amber-400 text-xs max-w-md mx-auto bg-amber-950/50 p-3 rounded-xl border border-amber-800">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-sm">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <span className="text-xs font-bold tracking-wider text-amber-400 uppercase bg-amber-950/50 px-3 py-1 rounded-full border border-amber-800">
                    Persona Identificada no Salão
                  </span>
                  <h3 className="text-xl font-bold text-zinc-100 mt-2 font-serif">
                    {recommendationResult.descricao_persona}
                  </h3>
                </div>
                <div className="text-xs text-zinc-400 bg-zinc-800 p-3 rounded-2xl border border-zinc-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {recommendationResult.recomendacoes.map((prod, index) => {
                ...
                return (
                  <div
                    key={prod.produto_id}
                    className={`bg-zinc-900 rounded-3xl border transition flex flex-col justify-between overflow-hidden relative ${
                      isTop1 
                        ? 'border-2 border-amber-600 shadow-lg' 
                        : 'border-zinc-800 shadow-sm'
                    }`}
                  >
                    {/* Badge do Top 1 */}
                    {isTop1 && (
                      <div className="bg-amber-600 text-zinc-100 text-xs font-bold px-4 py-1 text-center flex items-center justify-center space-x-1">
                    ...
                    <div className="p-6">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {/* Estrelas Estimadas */}
                        <div className="flex items-center space-x-1 bg-amber-950/50 px-2 py-1 rounded-lg border border-amber-800 text-amber-400 text-xs font-bold">
                          <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-500" />
                          <span>{prod.estrelas_estimadas}</span>
                        </div>
                      </div>

                      <h4 className="text-lg font-extrabold text-zinc-100 mb-2 leading-snug">
                        {prod.nome}
                      </h4>
                      ...
                      <div className="bg-zinc-800 rounded-2xl p-3 border border-zinc-800 mb-4">
                        <div className="text-[11px] font-bold text-zinc-300 flex items-center space-x-1 mb-1">
                    ...
                    <div className="p-4 bg-zinc-800/50 border-t border-zinc-800 mt-auto">
                      {foiAvaliado ? (
                          ...
                          <div className="flex justify-center items-center space-x-1.5">
                            {[1, 2, 3, 4, 5].map((nota) => (
                                <button
                                  ...
                                  className="w-11 h-11 min-h-[44px] min-w-[44px] rounded-full bg-zinc-900 hover:bg-zinc-700 border border-zinc-800 flex items-center justify-center text-sm font-bold text-zinc-300 hover:text-amber-400 transition active:scale-95 shadow-sm"
                                  title={`Dar nota ${nota}`}
                                >
                                {nota}★
                              </button>
            ...
            <div className="text-center pt-4">
              <button
                onClick={handleReset}
                className="inline-flex items-center justify-center w-full md:w-auto space-x-2 bg-amber-600 hover:bg-amber-700 text-white font-semibold text-sm px-6 py-3.5 rounded-xl transition shadow-md min-h-[44px]"
              >
      ...
      {/* Footer Discreto */}
      <footer className="border-t border-zinc-800 bg-zinc-900/50 text-zinc-500 text-xs py-4 text-center">
